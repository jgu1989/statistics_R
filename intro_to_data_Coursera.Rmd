...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_dd = median(arr_delay), q = IQR(arr_delay), n = n())
```
...
nycflights <- nycflights %>%
  mutate(arr_type = arr_delay<=0) %>%
  mutate(dep_type = dep_delay>0)

dim(filter(nycflights, arr_type=='TRUE' & dep_type=='TRUE')) / dim(filter(nycflights, arr_type=='TRUE'))
```